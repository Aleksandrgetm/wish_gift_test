...
<template>
    <article class="catalog-product-card">
        ...
        <div class="catalog-product-content">
            ...
            <h2>{{ product.name }}</h2>
            <div class="catalog-product-meta"><div><strong>{{ product.priceLabel }}</strong><del v-if="product.oldPrice">{{ product.oldPrice.toLocaleString('ru-RU') }} €</del></div><span :class="{ unavailable: !product.available }">{{ product.available ? 'Можно согласовать' : 'Под заказ' }}</span></div>
